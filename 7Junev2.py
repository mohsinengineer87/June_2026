...
# ── Official Elsevier/ESWA figure requirements ────────────────────────────────
# Source: https://www.sciencedirect.com/journal/expert-systems-with-applications/publish/guide-for-authors
# Halftone (photos, scatter, histogram):   300 DPI, single-col min 1063px
# Combination (line+halftone, tables):     500 DPI, single-col min 1772px
# Line drawings (bar, ROC, line charts):  1000 DPI, single-col min 3543px
# Recommended format: PNG or TIFF, RGB colour space
# Font: minimum 8pt after scaling to print size

_DPI_HALFTONE    = 300    # scatter plots, histograms, heatmaps
_DPI_COMBINATION = 500    # tables, mixed figures
_DPI_LINE        = 1000   # bar charts, line charts, ROC curves, pie charts
_DPI             = 300    # default fallback

_FW_SINGLE  = 3.54  # inches — single column (90mm, Elsevier spec: 1063px @ 300dpi)
_FW_DOUBLE  = 7.48  # inches — double column (190mm, Elsevier spec: 2244px @ 300dpi)
_FW_WIDE    = 7.48  # inches — full width (same as double for ESWA)
_FONT_BASE  = 9     # pt — axis labels (≥8pt required by Elsevier)
_FONT_TITLE = 10    # pt — figure title
_FONT_TICK  = 8     # pt — tick labels (8pt minimum)
_FONT_LEG   = 8     # pt — legend (8pt minimum)

# Colourblind-safe palette (recommended by Elsevier)
...
def _savefig(fig, dpi=None):
    """Save figure to base64 PNG — fully self-contained.
    dpi=None uses _DPI (300) for ZIP export.
    dpi=96 used for Claude API (keeps images under 2000px limit).
    """
    import io as _io_local
    import base64 as _b64_local
    plt, _ = _mpl()
    buf = _io_local.BytesIO()
    fig.savefig(buf, format="png", dpi=dpi or _DPI, bbox_inches="tight",
                ...
                try:
                    ...
                    with _zf_zip.ZipFile(zip_buf, "w", _zf_zip.ZIP_DEFLATED) as zf:
                        ...
                        for i, (label, b64) in enumerate(comp_figs_now):
                            ...
                            zf.writestr(fname, _b64_zip.b64decode(b64))
                            fig_count += 1
                        # Add per-run figures
                        for r in runs:
