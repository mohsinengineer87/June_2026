...
# ═══════════════════════════════════════════════════════════════════════════
# TAB: CLAUDE AI — Auto Results Analysis & Paper Assistant
# ═══════════════════════════════════════════════════════════════════════════

def _claude_get_client():
    """Return Anthropic client using Streamlit secrets or env var."""
    key = None
    try:
    ...
    if not key:
        st.error(
            ...
            "Create the file `.streamlit/secrets.toml` in your project folder with:\n"
...
def _claude_image_to_base64(img):
    buf = _io.BytesIO()
    img.save(buf, format="PNG")
    return _base64.standard_b64encode(buf.getvalue()).decode("utf-8")


def _claude_collect_context():
    """Pull live metrics from session_state so Claude gets exact numbers."""
    ss  = st.session_state
    ctx = {}
    ...
    if isinstance(ml, dict) and ml:
        ...
        ctx["ml_models"] = rows
        ml_only = [r for r in rows if "Baseline" not in r["Model"] and "Dummy" not in r["Model"]]
        if ml_only:
            best = max(ml_only, key=lambda r: float(r["R²"]) if r["R²"] != "—" else -1)
...
def _claude_format_context(ctx):
    lines = ["### Live Session Metrics (from your current run)\n"]
    if "n_assets"    in ctx: lines.append(f"- Assets simulated: {ctx['n_assets']}")
    if "n_actors"    in ctx: lines.append(f"- Threat actors: {ctx['n_actors']}")
    if "n_scenarios" in ctx: lines.append(f"- Scenarios generated: {ctx['n_scenarios']:,}")
    if "best_model"  in ctx:
        lines.append(f"- Best ML model: {ctx['best_model']} — R²={ctx['best_r2']} | MAE={ctx['best_mae']}")
    if "ml_models" in ctx:
        lines.append("\nML Regression Results:")
        for r in ctx["ml_models"]:
            lines.append(f"  {r['Model']}: R²={r['R²']} MAE={r['MAE']} RMSE={r['RMSE']} MAPE={r['MAPE%']}% CV_R²={r['CV R²']}")
    if "clf_models" in ctx:
        lines.append("\nAlerting Classifier Results:")
        for r in ctx["clf_models"]:
            lines.append(f"  {r['Model']}: F1={r['F1']} AUC={r['AUC']} PR-AUC={r['PR-AUC']}")
    if "scalability_slopes" in ctx:
        lines.append("\nEmpirical Complexity Slopes (log-log):")
        for stage, slope in ctx["scalability_slopes"].items():
            lines.append(f"  {stage}: β̂₁ = {slope}")
    if "live_cve_rows" in ctx:
        lines.append(f"\nLive Data: {ctx['live_cve_rows']} CVE rows | {ctx['known_exploited']} known-exploited")
    return "\n".join(lines)


_CLAUDE_PROMPTS = {
    "📊 Full Model Comparison": _textwrap.dedent("""
        ...
        Analyse the screenshot of results and produce:

        ## 1. Model Performance Table
        Extract all visible metrics (R², MAE, RMSE, MAPE, CV R²) ranked best to worst.

        ## 2. Champion Model
        Identify the best model with exact numbers and explain why it wins.

        ## 3. Baseline Gap
        Quantify ΔR² over the PASTA Formula Baseline. Is the ML improvement
        large enough to justify the added complexity to an ESWA reviewer?

        ## 4. Reviewer Risk Flags
        List any results a tough reviewer would question — suspiciously high R²,
        potential target leakage, missing confidence intervals, etc.

        ## 5. Paper-Ready Results Paragraph
        Write 3–4 sentences for Section 5.2 of the paper with exact numbers.
        No first person. Publication tone.

        ## 6. Before-Submission Recommendations
        Up to 3 concrete additions that would strengthen the results section.
    """),

    "🔍 SHAP Feature Attribution": _textwrap.dedent("""
        You are a cybersecurity ML explainability expert reviewing SHAP results
        from PASTA-ML (ESWA submission).

        {context}

        Analyse the SHAP plot in the screenshot:

        ## 1. Feature Ranking
        List features in descending SHAP |φ| order with values if visible.

        ## 2. Live API Features
        Flag which top features come from live APIs:
        cvss_weighted_avg → NVD | exploitability_score → FIRST EPSS | patch_compliance_inv → CISA KEV

        ## 3. Graph Topology Signal — RQ2 Answer
        Locate attack_path_length_inv. Its rank directly answers Research Question 2.
        State whether RQ2 is answered affirmatively based on what you see.

        ## 4. SOC Practitioner Interpretation
        In plain English: what does this plot tell a security analyst?

        ## 5. ESWA Figure Caption
        Write a publication-quality caption (≤3 sentences) for this figure.
    """),

    "🌐 Live Data Validation": _textwrap.dedent("""
        ...
        ## 1. Active Sources
        Which of the 8 sources are confirmed loaded?
        CISA KEV | FIRST EPSS | NVD CVE API 2.0 | MITRE ATT&CK |
        OASIS STIX/TAXII | CycloneDX SBOM | SPDX SBOM | NIST CVSS

        ## 2. Record Counts
        Extract all visible row/record counts. These are critical for Section 4.6.

        ## 3. Asset Table Summary
        Summarise WEB-001, DB-001 entries if visible (vuln counts, CVSS real, EPSS max, KEV count).

        ## 4. Section 4.6 Validation Statement
        Draft one sentence for the paper confirming live data ingestion with exact numbers.

        ## 5. Anomalies
        Flag anything unexpected — failed APIs, zero rows, fields missing — that
        could weaken the live-validation claim in peer review.
    """),

    "📈 Scalability Analysis": _textwrap.dedent("""
        You are a systems performance expert reviewing scalability benchmarks
        from PASTA-ML (ESWA submission).

        {context}

        Analyse the scalability output in the screenshot:

        ## 1. Complexity Classes
        Extract log-log slopes (β̂₁) for each phase. Are they consistent
        with the O(N^1.4) claim in the paper?

        ## 2. Runtime Table
        Reproduce all visible (N, time_s, memory_MB) data rows.

        ## 3. Bottleneck
        Which phase has the steepest slope and at what N does it dominate?

        ## 4. 5,000-Asset Claim Verification
        Does the chart support deployment at 5,000 assets on 16 GB RAM?
        Extrapolate the trend if needed.

        ## 5. ESWA Figure Caption
        Write a publication-quality caption for this scalability figure
        (≤3 sentences, cite the complexity class and hardware spec).
    """),

    "🧪 Permutation Test": _textwrap.dedent("""
        You are a statistical ML reviewer examining the permutation test
        from PASTA-ML (ESWA submission).
        ...
        ## 1. Test Summary
        State true R² vs. permuted R² and p-value. Does the null distribution
        fully exclude the true R²?

        ## 2. Target Leakage Assessment
        Does the R² collapse confirm the model learns genuine signal beyond
        formula re-encoding? Explain why this answers RQ1.

        ## 3. Reviewer Strength Rating
        Rate this evidence 1–5 for convincing a sceptical ESWA reviewer.
        Justify your rating.

        ## 4. Section 5.3 Paragraph
        Write 2–3 sentences for the paper describing this result with exact numbers.
    """),

    "📸 Figure Quality & Paper Advice": _textwrap.dedent("""
        You are a senior academic advisor preparing figures for an ESWA journal
        submission of PASTA-ML.

        {context}

        Review this screenshot from the Streamlit app:

        ## 1. Publication Readiness (rate 1–5)
        Score on: resolution/sharpness, font size (min 8pt), colour contrast,
        data-ink ratio, axis labels completeness.

        ## 2. ESWA Format Compliance
        ESWA requires 300 DPI minimum, TIFF/EPS preferred, single-column ~3.5 inches.
        Flag any compliance issues.

        ## 3. Specific Improvements
        List up to 5 concrete changes (font size, axis labels, legend, colour, gridlines).

        ## 4. Figure Caption
        Write a publication-quality caption (≤3 sentences).

        ## 5. Export Recommendation
        Should this be: (a) Plotly write_image scale=3, (b) redrawn in matplotlib,
        or (c) used as-is? Give a specific reason.
    """),
}
...
with tab_claude:
    ...
    st.markdown(
        ...
        "📸 Take a screenshot of any PASTA-ML tab, upload it here, and Claude will "
        "analyse it alongside your live session metrics to produce paper-ready insights, "
        "results paragraphs, and ESWA figure captions."
        "</div>",
        unsafe_allow_html=True
    )
    ...
    if not _ANTHROPIC_AVAILABLE:
        st.error(
            ...
            "Run this in your terminal and restart Streamlit:\n"
            "```\npip install anthropic\n```"
        )
    else:
        # ── Controls ──────────────────────────────────────────────────────────
        col_mode, col_toggle = st.columns([3, 1])
        with col_mode:
            _claude_mode = st.selectbox(
                "Analysis Mode",
                list(_CLAUDE_PROMPTS.keys()),
                help="Choose what Claude focuses on."
            )
        with col_toggle:
            _claude_use_ctx = st.toggle(
                "Include live metrics",
                value=True,
                help="Send your current session results to Claude alongside the screenshot."
            )

        # ── Upload ────────────────────────────────────────────────────────────
        _claude_uploaded = st.file_uploader(
            "📸 Upload a screenshot of any PASTA-ML results tab (PNG or JPG)",
            type=["png", "jpg", "jpeg"],
            help=(
        ...
        _claude_image = None
        if _claude_uploaded:
            _claude_image = _PILImage.open(_claude_uploaded)
            _preview = _claude_image.copy()
            _preview.thumbnail((900, 500))
            st.image(_preview, caption=_claude_uploaded.name, use_column_width=True)

        # ── Live context panel ─────────────────────────────────────────────────
        _claude_ctx = _claude_collect_context()
        if _claude_use_ctx:
            with st.expander("📊 Live session metrics being sent to Claude", expanded=False):
                if _claude_ctx:
                    _kpi_rows = []
                    if "n_assets"    in _claude_ctx: _kpi_rows.append({"Metric":"Assets",    "Value": _claude_ctx["n_assets"]})
                    if "n_scenarios" in _claude_ctx: _kpi_rows.append({"Metric":"Scenarios",  "Value": f"{_claude_ctx['n_scenarios']:,}"})
                    if "best_model"  in _claude_ctx:
                        _kpi_rows.append({"Metric":"Best model","Value": _claude_ctx["best_model"]})
                        _kpi_rows.append({"Metric":"Best R²",   "Value": _claude_ctx["best_r2"]})
                        _kpi_rows.append({"Metric":"Best MAE",  "Value": _claude_ctx["best_mae"]})
                    if _kpi_rows:
                        st.dataframe(pd.DataFrame(_kpi_rows), hide_index=True, use_container_width=True)
                    if "ml_models" in _claude_ctx:
                        st.markdown("**ML Regression Models**")
                        st.dataframe(pd.DataFrame(_claude_ctx["ml_models"]), hide_index=True, use_container_width=True)
                    if "clf_models" in _claude_ctx:
                        st.markdown("**Alerting Classifier Models**")
                        st.dataframe(pd.DataFrame(_claude_ctx["clf_models"]), hide_index=True, use_container_width=True)
                else:
                    st.info("No session metrics yet — run Steps 2–6 first. Claude will still analyse the screenshot.")

        # ── Analyse button ─────────────────────────────────────────────────────
        st.divider()
        if not _claude_image:
            st.info("👆 Upload a screenshot above to enable analysis.")

        if st.button(
            "🧠 Analyse with Claude",
            type="primary",
            disabled=(_claude_image is None),
            use_container_width=True,
            key="claude_analyse_btn"
        ):
            _ctx_str = _claude_format_context(_claude_ctx) if (_claude_use_ctx and _claude_ctx) else ""
            _prompt  = _CLAUDE_PROMPTS[_claude_mode].format(context=_ctx_str)

            with st.spinner("Claude is reading your results…"):
                try:
                    _client = _claude_get_client()
                            ...
                            "content": [
                                {
                                    ...
                                    "source": {
                                        ...
                                        "data":       _claude_image_to_base64(_claude_image),
                                    },
                                },
                    ...
                    st.session_state["_claude_analysis"] = _resp.content[0].text
                    st.session_state["_claude_mode_used"] = _claude_mode
                    st.session_state["_claude_ts"]        = datetime.now().strftime("%d %b %Y %H:%M")
                except Exception as _ce:
                    st.error(f"Claude API error: {_ce}")

        # ── Show result ────────────────────────────────────────────────────────
        if st.session_state.get("_claude_analysis"):
            _analysis  = st.session_state["_claude_analysis"]
            _mode_used = st.session_state.get("_claude_mode_used", "")
            _ts        = st.session_state.get("_claude_ts", "")

            st.success(f"✅ Analysis ready — {_mode_used} · {_ts}")
            st.divider()
            ...
            with _dl1:
                st.download_button(
                    "📥 Download as .txt",
                    data=_analysis.encode("utf-8"),
                    file_name=f"claude_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
                    mime="text/plain",
                    use_container_width=True,
                )
            with _dl2:
                _md = (f"# PASTA-ML Claude Analysis\n**Mode:** {_mode_used}  \n"
                       f"**Generated:** {_ts}\n\n---\n\n{_analysis}")
                st.download_button(
                    "📄 Download as .md (paste into paper)",
                    data=_md.encode("utf-8"),
                    file_name=f"claude_{datetime.now().strftime('%Y%m%d_%H%M')}.md",
                    mime="text/markdown",
                    use_container_width=True,
                )

        pass  # screenshot guide removed — auto-capture replaces it


# ─────────────────────────────────────────────────────────────────────────────
# AUTO-CAPTURE: build figures directly from session_state (no screenshots needed)
# ─────────────────────────────────────────────────────────────────────────────

def _fig_to_b64(fig):
    """Try kaleido first; fall back to matplotlib rendering."""
    try:
        import kaleido  # noqa
        buf = _io.BytesIO()
        fig.write_image(buf, format="png", scale=2, width=900, height=500)
        buf.seek(0)
        return _base64.standard_b64encode(buf.read()).decode("utf-8")
    except Exception:
        return None  # caller will use matplotlib fallback


def _df_to_b64_mpl(df, title=""):
    """Kaleido-free: render DataFrame as PNG table via matplotlib."""
    try:
        ...
        fig, ax = plt.subplots(figsize=(max(6, 0.9*n_cols), max(1.2, 0.35*(n_rows+2))))
        ax.axis("off")
        if title:
...
def _chart_to_b64_mpl(x_vals, y_dict, title, xlabel, ylabel, log_axes=False):
    """Kaleido-free: render line chart via matplotlib. y_dict={label: y_array}"""
    try:
        import matplotlib; matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        colors = ["#1a73e8","#34A853","#EA4335","#FBBC04","#9C27B0","#00BCD4"]
        fig, ax = plt.subplots(figsize=(8, 4))
        for (label, y_vals), color in zip(y_dict.items(), colors):
...
def _df_to_fig(df, title):
    """Render a DataFrame as a Plotly table figure."""
    df = df.copy().round(4) if hasattr(df, "round") else df
    col_vals = [df[c].astype(str).tolist() for c in df.columns]
    fig = go.Figure(data=[go.Table(
        header=dict(
            values=[f"<b>{c}</b>" for c in df.columns],
            fill_color="#1a73e8", font=dict(color="white", size=11),
    ...
    fig.update_layout(title=title, margin=dict(l=0, r=0, t=40, b=0),
                      height=max(180, min(40 * len(df) + 80, 600)))
    return fig


def _build_auto_images():
    """
    Build all result figures from session_state and return a list of
    (label, base64_png) tuples ready to send to Claude.
    No screenshots, no uploads — everything rendered from live data.
    """
    images = []
    ss = st.session_state

    # ── 1. ML Regression metrics table ───────────────────────────────────────
    ml = ss.get("ml_results")
    if isinstance(ml, dict) and ml:
        rows = []
        for name, r in ml.items():
        ...
        b64 = _df_to_b64_mpl(df_ml, "Step 5 — ML Regression Model Comparison")
        if b64:
            images.append(("ML Regression Metrics Table", b64))

        # Predicted vs Actual scatter (best ML model)
        ml_only = {n: r for n, r in ml.items()
                   if isinstance(r, dict) and r.get("model_kind") == "ml"}
        if ml_only:
            best_name = max(ml_only, key=lambda n: float(ml_only[n].get("r2", -1)))
            best = ml_only[best_name]
            if "y_test" in best and "y_pred" in best:
                y_t = np.array(best["y_test"])
                y_p = np.array(best["y_pred"])
                fig_sc = go.Figure()
                fig_sc.add_trace(go.Scatter(x=y_t, y=y_p, mode="markers",
                    marker=dict(color="#2980b9", size=4, opacity=0.6),
                    name="Predictions"))
                ...
                fig_sc.update_layout(
                    title=f"{best_name}: Predicted vs Actual Risk Score "
                ...
                b64 = _fig_to_b64(fig_sc)
                if b64 is None:  # kaleido not available — use matplotlib
                    try:
                        import matplotlib; matplotlib.use("Agg")
                        import matplotlib.pyplot as plt
                        fig_m, ax_m = plt.subplots(figsize=(6,5))
                        ax_m.scatter(y_t, y_p, s=8, alpha=0.5, color="#2980b9")
                        mn_m, mx_m = min(y_t.min(), y_p.min()), max(y_t.max(), y_p.max())
                        ax_m.plot([mn_m, mx_m], [mn_m, mx_m], "r--", linewidth=1.5)
                        ax_m.set_xlabel("Actual Risk Score"); ax_m.set_ylabel("Predicted Risk Score")
                        ax_m.set_title(f"{best_name}: Predicted vs Actual\nR²={best.get('r2','?')} MAE={best.get('mae','?')}")
                        ax_m.grid(True, alpha=0.3); plt.tight_layout()
                        buf_m = _io.BytesIO(); plt.savefig(buf_m, format="png", dpi=150); plt.close(fig_m); buf_m.seek(0)
                        b64 = _base64.standard_b64encode(buf_m.read()).decode("utf-8")
                    except Exception: b64 = None
                if b64:
                    images.append((f"{best_name} — Predicted vs Actual", b64))
        ...
        if best_shap:
            ...
            shap_df  = pd.DataFrame({"Feature": fnames, "Mean |SHAP|": mean_abs})
            shap_df  = shap_df.sort_values("Mean |SHAP|")
            fig_shap = px.bar(shap_df, x="Mean |SHAP|", y="Feature",
                              orientation="h",
                              color="Mean |SHAP|",
                              color_continuous_scale="Blues",
                              title=f"SHAP Feature Importance — {name}")
            fig_shap.update_layout(height=420, margin=dict(l=0, r=0, t=50, b=0),
                                   coloraxis_showscale=False)
            b64 = _fig_to_b64(fig_shap)
            if b64 is None:
                b64 = _chart_to_b64_mpl(
                    shap_df["Mean |SHAP|"].values,
                    {"Mean |SHAP|": shap_df["Mean |SHAP|"].values},
                    f"SHAP Feature Importance — {name}",
                    "Mean |SHAP|", "Feature")
            if b64:
                images.append(("SHAP Feature Importance", b64))

        # Permutation test
        for name, r in ml.items():
            if isinstance(r, dict) and r.get("perm_r2_scores") is not None:
                perm_scores = np.array(r["perm_r2_scores"])
                true_r2     = float(r.get("r2", 0))
                fig_perm = go.Figure()
                fig_perm.add_trace(go.Histogram(
                    x=perm_scores, nbinsx=20,
                    marker_color="#e74c3c", name="Permuted R²"))
                fig_perm.add_vline(x=true_r2, line_dash="dash",
                                   line_color="#2980b9", line_width=2,
                                   annotation_text=f"True R²={true_r2:.3f}",
                                   annotation_position="top right")
                fig_perm.update_layout(
                    title=f"Permutation Test — {name} (100 runs)",
                    xaxis_title="Permuted R²",
                    yaxis_title="Frequency",
                    height=400, margin=dict(l=0, r=0, t=50, b=0))
                b64 = _fig_to_b64(fig_perm)
                if b64 is None:
                    try:
                        import matplotlib; matplotlib.use("Agg")
                        import matplotlib.pyplot as plt
                        fig_mp, ax_mp = plt.subplots(figsize=(7, 3.5))
                        ax_mp.hist(perm_scores, bins=20, color="#e74c3c", edgecolor="white")
                        ax_mp.axvline(true_r2, color="#2980b9", linestyle="--", linewidth=2,
                                      label=f"True R²={true_r2:.3f}")
                        ax_mp.set_xlabel("Permuted R²"); ax_mp.set_ylabel("Frequency")
                        ax_mp.set_title(f"Permutation Test — {name} (100 runs)")
                        ax_mp.legend(); ax_mp.grid(True, alpha=0.3); plt.tight_layout()
                        buf_mp = _io.BytesIO(); plt.savefig(buf_mp, format="png", dpi=150); plt.close(fig_mp); buf_mp.seek(0)
                        b64 = _base64.standard_b64encode(buf_mp.read()).decode("utf-8")
                    except Exception: b64 = None
                if b64:
                    images.append(("Permutation Test", b64))
                break

    # ── 2. Alerting classifier metrics ───────────────────────────────────────
    clf = ss.get("clf_results")
    if isinstance(clf, dict) and clf and "error" not in clf:
        rows = []
        for name, r in clf.items():
        ...
        b64 = _df_to_b64_mpl(df_clf, "Step 5b — Alerting Classifier Comparison")
        if b64:
            images.append(("Alerting Classifier Metrics Table", b64))

        # ROC curves — all models on one chart
        fig_roc_all = go.Figure()
        colors_roc  = ["#2980b9","#27ae60","#e74c3c","#8e44ad","#e67e22","#1abc9c"]
        for (name, r), col in zip(clf.items(), colors_roc):
            if isinstance(r, dict) and "y_test" in r and "y_proba" in r:
                try:
                    fpr, tpr, _ = roc_curve(np.array(r["y_test"]),
                                            np.array(r["y_proba"]))
                    fig_roc_all.add_trace(go.Scatter(
                        x=fpr, y=tpr, mode="lines",
                        name=f"{name} (AUC={r.get('roc_auc','?'):.3f})",
                        line=dict(color=col, width=2)))
                except Exception:
                    pass
        ...
        fig_roc_all.update_layout(
            title="ROC Curves — All Alerting Classifiers",
            xaxis_title="False Positive Rate",
            yaxis_title="True Positive Rate",
            height=500, margin=dict(l=0, r=0, t=50, b=0),
            legend=dict(x=0.55, y=0.05))
        b64 = _fig_to_b64(fig_roc_all)
        if b64 is None:
            # matplotlib ROC fallback
            try:
                import matplotlib; matplotlib.use("Agg")
                import matplotlib.pyplot as plt
                colors_m = ["#2980b9","#27ae60","#e74c3c","#8e44ad","#e67e22","#1abc9c"]
                fig_mr, ax_mr = plt.subplots(figsize=(7, 5))
                for (nm, r), cm in zip(clf.items(), colors_m):
                    if isinstance(r, dict) and "y_test" in r and "y_proba" in r:
                        try:
                            from sklearn.metrics import roc_curve as _rc
                            fpr_m, tpr_m, _ = _rc(np.array(r["y_test"]), np.array(r["y_proba"]))
                            ax_mr.plot(fpr_m, tpr_m, color=cm, linewidth=2,
                                       label=f"{nm} (AUC={r.get('roc_auc','?'):.3f})")
                        except Exception: pass
                ax_mr.plot([0,1],[0,1],"--",color="grey",label="Random")
                ax_mr.set_xlabel("FPR"); ax_mr.set_ylabel("TPR")
                ax_mr.set_title("ROC Curves — All Alerting Classifiers")
                ax_mr.legend(fontsize=7); ax_mr.grid(True, alpha=0.3); plt.tight_layout()
                buf_mr = _io.BytesIO(); plt.savefig(buf_mr, format="png", dpi=150); plt.close(fig_mr); buf_mr.seek(0)
                b64 = _base64.standard_b64encode(buf_mr.read()).decode("utf-8")
            except Exception: b64 = None
        if b64:
            images.append(("ROC Curves — All Classifiers", b64))

    # ── 3. Scalability benchmark (uses bench_multi_seed_agg — actual Step 6 key) ──
    bench = ss.get("bench_multi_seed_agg")
    if bench is None:
        bench = ss.get("bench_results")   # legacy fallback
    if isinstance(bench, pd.DataFrame) and len(bench) > 0:
        # Table — matplotlib, no kaleido needed
        display_cols = [c for c in bench.columns
                        if any(x in c.lower() for x in
                               ["n", "time", "mem", "scenario", "feature", "train",
                                "total", "mean", "std", "ci"])][:8]
        if display_cols:
            b64 = _df_to_b64_mpl(bench[display_cols].round(3),
                                  "Step 6 — Scalability Benchmark (multi-seed)")
            if b64:
                images.append(("Scalability Benchmark Table", b64))

        # Log-log chart — matplotlib, no kaleido needed
        n_col  = next((c for c in bench.columns if c.upper() == "N"), None)
        t_cols = [c for c in bench.columns
                  if any(x in c.lower() for x in ["total_time", "time_mean", "time_s",
                                                    "scenario", "feature", "train"])
                  and c != n_col][:4]
        if n_col and t_cols:
            try:
                x_n  = bench[n_col].values.astype(float)
                mask = x_n > 0
                y_dict = {}
                for col in t_cols:
                    y_vals = bench[col].values.astype(float)
                    if (y_vals[mask] > 0).any():
                        y_dict[col] = y_vals
                if y_dict:
                    b64 = _chart_to_b64_mpl(
                        x_n, y_dict,
                        "Scalability — Runtime by Phase (log-log)",
                        "N (Assets)", "Time (s)", log_axes=True)
                    if b64:
                        images.append(("Scalability Log-Log Chart", b64))
            except Exception:
                pass

    # Also pull complexity fits if available
    ...
    if isinstance(fits, dict) and fits:
        ...
        for stage, info in fits.items():
            if isinstance(info, dict):
                fit_rows.append({
                    "Stage":    stage,
                    "Slope β̂": round(float(info.get("slope", 0)), 3),
                    "CI lo":    round(float(info.get("slope_ci_lo", 0)), 3),
                    "CI hi":    round(float(info.get("slope_ci_hi", 0)), 3),
                    "R²":       round(float(info.get("r_squared", 0)), 4),
                })
        if fit_rows:
            b64 = _df_to_b64_mpl(pd.DataFrame(fit_rows),
                                  "Step 6 — Complexity Fit (O(N^k) per phase)")
            if b64:
                images.append(("Complexity Slopes Table", b64))

    # ── 4. Risk distribution ──────────────────────────────────────────────────
    feat = ss.get("features")
    if feat is not None and "risk_score" in feat.columns:
        fig_hist = px.histogram(feat, x="risk_score", nbins=40,
                                color_discrete_sequence=["#2980b9"],
                                title="Risk Score Distribution (All Scenarios)")
        fig_hist.update_layout(height=380, margin=dict(l=0, r=0, t=50, b=0),
                               xaxis_title="Risk Score (0–10)",
                               yaxis_title="Count")
        b64 = _fig_to_b64(fig_hist)
        if b64 is None:
            b64 = _chart_to_b64_mpl(
                feat["risk_score"].values, {"Count": feat["risk_score"].values},
                "Risk Score Distribution", "Risk Score (0-10)", "Count")
        if b64:
            images.append(("Risk Score Distribution", b64))

        if "asset_type" in feat.columns:
            grp = feat.groupby("asset_type")["risk_score"].mean().reset_index()
            grp.columns = ["Asset Type", "Mean Risk"]
            grp = grp.sort_values("Mean Risk", ascending=True)
            fig_at = px.bar(grp, x="Mean Risk", y="Asset Type",
                            orientation="h", color="Mean Risk",
                            color_continuous_scale="RdYlGn_r",
                            title="Mean Predicted Risk by Asset Type")
            fig_at.update_layout(height=380, margin=dict(l=0, r=0, t=50, b=0),
                                 coloraxis_showscale=False)
            b64 = _fig_to_b64(fig_at)
            if b64 is None:
                b64 = _df_to_b64_mpl(grp.sort_values("Mean Risk", ascending=False),
                                      "Mean Predicted Risk by Asset Type")
            if b64:
                images.append(("Mean Risk by Asset Type", b64))

    return images


def _build_full_prompt(images, ctx):
    """Build the master prompt that references all captured figures."""
    ctx_str = _claude_format_context(ctx) if ctx else ""
    figure_list = "\n".join(f"  Figure {i+1}: {label}"
                            for i, (label, _) in enumerate(images))
    return f"""You are an expert ML researcher and academic writing assistant helping
to prepare the results section of a paper titled:

"PASTA-ML: A Scalable Machine Learning-Integrated Threat Modeling Framework
for Large-Scale Cyber-Physical Systems"

This paper is being submitted to Expert Systems with Applications (ESWA).

{ctx_str}

You have been provided {len(images)} figures automatically captured from the
live running Streamlit application:

{figure_list}

Analyse ALL figures carefully and produce a complete, structured report:

---

## 1. Executive Results Summary
3–4 sentences summarising the key quantitative findings across all results.
Write in third person, publication tone. Include the top R², F1, AUC numbers.

---

## 2. ML Regression Performance (answers RQ1)
- Rank all models by R² (best to worst) with exact numbers
- Quantify ΔR² of best ML model vs PASTA Formula Baseline
- Comment on CV R² stability (std deviation)
- Flag any overfitting concerns

---

## 3. Permutation Test Interpretation (answers RQ1)
- State true R² vs permuted R² and p-value if visible
- Confirm whether this rules out formula re-learning
- Rate the evidence strength for an ESWA reviewer (1–5)

---

## 4. SHAP Feature Attribution (answers RQ2)
- List top 5 features by mean |SHAP| with values
- Identify which come from live APIs (NVD/EPSS/KEV)
- State rank of attack_path_length_inv — does this confirm
  graph topology adds independent signal (RQ2)?

---

## 5. Alerting Classifier Performance
- Rank classifiers by F1 and AUC
- Comment on the ROC curve separation
- Note best F1/AUC values for the paper

---

## 6. Scalability Analysis
- State observed complexity class (O(N^?)) per phase
- Does the trend support 5,000-asset deployment?
- Note any phase that becomes a bottleneck at large N

---

## 7. Risk Distribution Insights
- Describe the shape of the risk score distribution
- Which asset types carry the highest predicted risk?
- Is this consistent with the CISA KEV / MITRE ATT&CK priorities cited in the paper?

---

## 8. Paper-Ready Section 5 Draft
Write a complete Results section draft (≈400 words) covering RQ1, RQ2, RQ3
with exact numbers from the figures. Use academic tone, third person,
past tense. Structure: regression results → permutation test → ablation/SHAP
→ alerting classifier → scalability. This should be close to submission-ready.

---

## 9. Reviewer Risk Flags
List up to 5 things a tough ESWA reviewer might challenge, and suggest
a one-sentence rebuttal for each.

---

## 10. Figure Captions
Write a publication-quality caption (≤3 sentences each) for every figure
you can see. Number them to match the figure list above.
"""


with tab_claude:
    st.subheader("✨ Claude AI — Automatic Results Analysis & Paper Assistant")
    st.markdown(
        "<div style='background:#f0f6ff;border-left:4px solid #1a73e8;"
        "padding:12px 16px;border-radius:6px;margin-bottom:14px;'>"
        "🤖 <b>No screenshots needed.</b> Claude automatically reads all your results "
        "directly from the running app — ML metrics, SHAP plots, ROC curves, scalability "
        "benchmarks — and produces a complete paper-ready Results section draft, "
        "figure captions, and reviewer responses."
        "</div>",
        unsafe_allow_html=True
    )
    ...
    if not _ANTHROPIC_AVAILABLE:
        st.error(
            "❌ `anthropic` package not installed.\n\n"
            "Add `anthropic>=0.25.0` to your `requirements.txt` and redeploy."
        )
        st.stop()

    # ── Status check ──────────────────────────────────────────────────────────
    ss = st.session_state
    has_ml    = isinstance(ss.get("ml_results"),   dict) and bool(ss.get("ml_results"))
    has_clf   = isinstance(ss.get("clf_results"),  dict) and "error" not in ss.get("clf_results", {})
    has_bench = (isinstance(ss.get("bench_multi_seed_agg"), pd.DataFrame) or
                  isinstance(ss.get("bench_complexity_fits"), dict))
    has_feat  = ss.get("features") is not None

    st.markdown("#### ✅ Pipeline Status")
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("Step 4 Features",   "✅ Ready"    if has_feat  else "⚠️ Not run")
    s2.metric("Step 5 ML Models",  "✅ Ready"    if has_ml    else "⚠️ Not run")
    s3.metric("Step 5b Alerting",  "✅ Ready"    if has_clf   else "⚠️ Not run")
    s4.metric("Step 6 Scalability","✅ Ready"    if has_bench else "⚠️ Not run")

    if not any([has_ml, has_clf, has_bench, has_feat]):
        st.warning(
            "⚠️ No results found yet. Run Steps 2 → 3 → 4 → 5 → 5b → 6 first, "
            "then come back here to generate the paper analysis."
        )
    else:
        # Count what will be captured
        _preview_images = _build_auto_images()
        st.success(
            f"🎯 **{len(_preview_images)} figures** ready to send to Claude — "
            f"automatically captured from your current session."
        )

        # Show what will be sent
        with st.expander(f"📋 Figures that will be analysed ({len(_preview_images)})",
                         expanded=False):
            for i, (label, _) in enumerate(_preview_images):
                st.markdown(f"  **Figure {i+1}:** {label}")

        st.divider()

        # ── Main analyse button ───────────────────────────────────────────────
        if st.button(
            "🧠 Generate Full Paper Analysis with Claude",
            type="primary",
            use_container_width=True,
            key="claude_auto_btn"
        ):
            with st.spinner(
                f"Sending {len(_preview_images)} figures to Claude and generating "
                f"paper analysis… this takes ~30 seconds…"
            ):
                try:
                    images = _build_auto_images()
                    ctx    = _claude_collect_context()
                    prompt = _build_full_prompt(images, ctx)

                    # Build content blocks — all figures + prompt
                    content = []
                    ...
                    st.session_state["_claude_analysis"] = analysis
                    st.session_state["_claude_ts"]       = datetime.now().strftime(
                        "%d %b %Y %H:%M")
                    st.session_state["_claude_n_figs"]   = len(images)

                except Exception as _ce:
                    st.error(f"Claude API error: {_ce}")

        # ── Display result ────────────────────────────────────────────────────
        if ss.get("_claude_analysis"):
            analysis = ss["_claude_analysis"]
            ts       = ss.get("_claude_ts", "")
            n_figs   = ss.get("_claude_n_figs", "?")

            st.success(f"✅ Analysis complete — {n_figs} figures analysed · {ts}")
            st.divider()
            ...
            with dl1:
                st.download_button(
                    ...
                    file_name=f"pasta_ml_paper_analysis_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
                    mime="text/plain",
                    use_container_width=True,
                )
            with dl2:
                md_out = (
                    f"# PASTA-ML — Claude Paper Analysis\n"
                    f"**Generated:** {ts}  \n"
                    f"**Figures analysed:** {n_figs}\n\n---\n\n{analysis}"
                )
                st.download_button(
                    "📄 Download as Markdown (.md)",
                    data=md_out.encode("utf-8"),
                    file_name=f"pasta_ml_paper_analysis_{datetime.now().strftime('%Y%m%d_%H%M')}.md",
                    mime="text/markdown",
                    use_container_width=True,
                )
