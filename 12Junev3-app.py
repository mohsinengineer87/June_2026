...
/* ── Prevent scroll-to-top on sidebar input changes ──────────────────────── */
.main .block-container {{ scroll-behavior: auto !important; }}
section[data-testid="stSidebar"] {{ overflow-y: auto !important; }}
/* Compact number_input in sidebar */
section[data-testid="stSidebar"] .stNumberInput > div {{ margin-bottom: 0.2rem; }}
section[data-testid="stSidebar"] .stNumberInput label {{ font-size: 0.82rem !important; }}
section[data-testid="stSidebar"] .stNumberInput input {{
    padding: 0.2rem 0.4rem !important; font-size: 0.85rem !important; }}
/* Remove spin buttons clutter */
...
# Extra keys for live data feeds — never reset on rerun
_live_defaults = {
    "live_kev_df":          None,
    "live_epss_df":         None,
    "live_nvd_df":          None,
    "bench_multi_seed_long":None,
    "bench_multi_seed_agg": None,
    "bench_complexity_fits":None,
    "strong_scaling_df":    None,
    "weak_scaling_df":      None,
    "incremental_df":       None,
    "approx_centrality_df": None,
    "saved_runs":           [],
}
...
def load_kaggle_enriched_dataset(uploaded_file):
    ...
    try:
        ...
        out = pd.DataFrame()
        out["cve_id"]               = _col("cve_id", "").astype(str)
        out["cvss_score"]           = pd.to_numeric(_col("base_score",            0), errors="coerce").fillna(0.0).clip(0, 10)
        out["cvss_severity"]        = _col("base_severity", "MEDIUM").astype(str).str.upper()
        out["exploitability_score"] = pd.to_numeric(_col("exploitability_score",  0), errors="coerce").fillna(0.0).clip(0, 10)
        out["impact_score"]         = pd.to_numeric(_col("impact_score",          0), errors="coerce").fillna(0.0).clip(0, 10)
        out["epss_score"]           = pd.to_numeric(_col("epss_score",            0), errors="coerce").fillna(0.0).clip(0, 1)
        out["epss_percentile"]      = pd.to_numeric(_col("epss_perc",             0), errors="coerce").fillna(0.0).clip(0, 1)
        # cisa_kev is bool in the raw file; coerce robustly
        kev_raw = _col("cisa_kev", False)
        out["known_exploited"]      = kev_raw.map(
            lambda x: 1 if str(x).strip().lower() in ("true","1","yes") else 0
        ).astype(int)
        out["attack_vector"]        = _col("attack_vector",       "NETWORK").astype(str).str.upper()
        out["attack_complexity"]    = _col("attack_complexity",   "LOW").astype(str).str.upper()
        out["privilege_required"]   = _col("privileges_required", "NONE").fillna("NONE").astype(str).str.upper()
        out["scope"]                = _col("scope",               "UNCHANGED").fillna("UNCHANGED").astype(str).str.upper()
        out["confidentiality_impact"] = _col("confidentiality_impact", "NONE").astype(str).str.upper()
        out["integrity_impact"]       = _col("integrity_impact",       "NONE").astype(str).str.upper()
        out["availability_impact"]    = _col("availability_impact",    "NONE").astype(str).str.upper()
        out["published_date"]       = _col("published_date", "").astype(str)
...
def enrich_assets_with_vulnerabilities(asset_df, vulns_df, sbom_df=None):
    ...
    if not vulns.empty:
        ...
        if "asset_id" in vulns.columns:
            ...
            if "vuln_count_real" in assets.columns:
...
# Scroll main content down after every sidebar interaction
...
(function() {
    function scrollDown() {
        var p = window.parent;
        if (!p) return;
        // Scroll down to show content tabs (below header)
        if (p.scrollY < 50) {
            p.scrollTo({ top: 300, behavior: 'smooth' });
        }
        // Also target Streamlit's internal scroll containers
        var sels = ['section.main', '.main .block-container',
                    '[data-testid="stAppViewContainer"]'];
        sels.forEach(function(sel) {
            var els = p.document.querySelectorAll(sel);
            els.forEach(function(el) {
                if (el.scrollTop < 50) el.scrollTop = 300;
            });
        });
    }
...
with st.sidebar:
    ...
    # ── ENTERPRISE RUN PRESETS ───────────────────────────────────────────────
    st.markdown("### 🎯 Enterprise Run Presets")
    st.caption("One click sets ALL inputs. Includes Fast Mode for Step 6 scalability.")

    # Asset mix order (mix_0..mix_6):
    # [Cloud VM, IoT Device, Database Server, Network Device, Enterprise App, SCADA/ICS, Endpoint]
    _PRESETS = {
        "fin": {
            "emoji": "🏦", "title": "Financial Services",
            "sub": "Bank / FinTech",   "color": "#1a73e8",
            "proves": "RQ1 — DB assets score highest risk",
            "n_assets": 300, "rng_seed": 42,
            "mix": [20, 5, 35, 10, 20, 0, 10],
            "actors": ["Cybercriminal", "Insider Threat", "APT Group"],
            "n_scenarios": 1000, "mc_n_sims": 200,
            "bench_max": 300, "bench_pts": 4, "note": "",
        },
        "ics": {
            "emoji": "🏭", "title": "Critical Infrastructure",
            "sub": "Energy / OT / ICS",  "color": "#EA4335",
            "proves": "RQ2 — Graph topology signal rises with ICS",
            "n_assets": 300, "rng_seed": 42,
            "mix": [5, 10, 15, 20, 10, 35, 5],
            "actors": ["Nation-State", "APT Group", "Hacktivist"],
            "n_scenarios": 1000, "mc_n_sims": 200,
            "bench_max": 300, "bench_pts": 4, "note": "",
        },
        "hlt": {
            "emoji": "🏥", "title": "Healthcare",
            "sub": "Hospital / Pharma + Live Data",  "color": "#34A853",
            "proves": "RQ3 — Live CVE data validates model",
            "n_assets": 300, "rng_seed": 42,
            "mix": [15, 30, 20, 10, 15, 5, 5],
            "actors": ["Cybercriminal", "APT Group", "Insider Threat"],
            "n_scenarios": 1000, "mc_n_sims": 200,
            "bench_max": 300, "bench_pts": 4,
            "note": "⚠️ Real Data + CTI tab → Build live bundle → Upload Kaggle CSV → Train classifier — do this BEFORE running pipeline.",
        },
    }

    def _apply_preset(p, key=""):
        ss = st.session_state
        ss["_n_assets_preset"]    = p["n_assets"]
        ss["_rng_seed_preset"]    = p["rng_seed"]
        ss["_n_scenarios_preset"] = p["n_scenarios"]
        ss["_mc_n_sims_preset"]   = p["mc_n_sims"]
        ss["_bench_max_preset"]   = min(p["bench_max"], 10000)
        ss["_bench_pts_preset"]   = p.get("bench_pts", 4)
        ss["_actors_preset"]      = p["actors"]
        for i, v in enumerate(p["mix"]):
            ss[f"mix_{i}"] = v
        ss["_preset_active"] = True
        ss["_preset_key"]    = key
        ss["_preset_note"]   = p.get("note", "")
        ss["_preset_title"]  = p["title"]

    # Draw preset cards
    _c1, _c2, _c3 = st.columns(3)
    for _col, (_key, _p) in zip([_c1, _c2, _c3], _PRESETS.items()):
        _active = (st.session_state.get("_preset_key") == _key
                   and st.session_state.get("_preset_active", False))
        with _col:
            _border = "#34A853" if _active else _p["color"]
            _bg     = "#f0fff4" if _active else "#fafafa"
            st.markdown(
                "<div style=\"border:2px solid " + _border + ";border-radius:10px;"
                "padding:10px 8px;background:" + _bg + ";text-align:center;\">"
                "<div style=\"font-size:1.5rem\">" + _p["emoji"] + "</div>"
                "<div style=\"font-weight:700;font-size:0.84rem;color:" + _p["color"] + ";margin:4px 0 2px\">"
                + _p["title"] + "</div>"
                "<div style=\"font-size:0.72rem;color:#666;margin-bottom:5px\">" + _p["sub"] + "</div>"
                "<div style=\"font-size:0.68rem;color:#333;background:#f0f4ff;"
                "border-radius:4px;padding:3px 5px\">📋 " + _p["proves"] + "</div>"
                "</div>",
                unsafe_allow_html=True)
            if st.button(
                "✅ Active" if _active else "▶ Select " + _p["emoji"],
                key=f"preset_{_key}",
                use_container_width=True,
                type="primary" if _active else "secondary",
                help=(_p["title"] + " | Assets:" + str(_p["n_assets"])
                      + " | Bench Max N:" + str(_p["bench_max"])
                      + " | Actors:" + ", ".join(_p["actors"]))):
                _apply_preset(_p, key=_key)
                st.rerun()

    # Active preset status + workflow
    if st.session_state.get("_preset_active"):
        _title = st.session_state.get("_preset_title", "")
        _note  = st.session_state.get("_preset_note", "")
        _key   = st.session_state.get("_preset_key", "")
        if _note:
            st.warning(f"✅ **{_title}** loaded.\n\n{_note}")
        else:
            st.success(f"✅ **{_title}** loaded — all inputs set below.")
        # Step-by-step workflow
        _wf = ["Step 2 → Environment Simulation",
               "Step 3 → Scenario Generation",
               "Step 4 → Feature Engineering",
               "Step 5 → ML Risk Estimation",
               "Step 5b → MC Alerting",
               "Step 6 → Scalability (click ⚡ Fast Mode first)",
               "✨ Claude AI tab → Save Run"]
        if _key == "hlt":
            _wf.insert(0, "Real Data + CTI → Load live data FIRST")
        st.markdown("**Run in this order:**")
        for _si, _step in enumerate(_wf):
            st.markdown(f"&nbsp;&nbsp;**{_si+1}.** {_step}")
        if st.button("✖ Clear preset", key="clear_preset_btn"):
            st.session_state["_preset_active"] = False
            st.session_state["_preset_key"]    = ""
            st.rerun()
    ...
    n_assets = st.number_input(
        "Total Assets", min_value=20, max_value=2000,
        value=int(st.session_state.get("_n_assets_preset", 150)), step=10,
        help="Number of synthetic assets to simulate. Larger N = more realistic but slower. Pipeline scales O(N^1.4).")
    rng_seed  = st.number_input("Random Seed",
        value=int(st.session_state.get("_rng_seed_preset", 42)), step=1,
    ...
    st.caption("Set % for each asset type. Total should sum to 100.")
    mix_raw = {}
    # Use preset mix if active, else defaults
    _preset_mix = [st.session_state.get(f"mix_{i}") for i in range(7)]
    _mix_defaults = _preset_mix if all(v is not None for v in _preset_mix) else [25, 15, 15, 10, 15, 10, 10]
    asset_list = list(ASSET_TYPES.keys())
    # Use number_input instead of slider — prevents scroll-to-top on change
    mix_cols = st.columns(2)
    for i, at in enumerate(asset_list):
        col = mix_cols[i % 2]
        mix_raw[at] = col.number_input(
            f"{ASSET_TYPES[at]['icon']} {at[:12]}",
            min_value=0, max_value=100,
            value=_mix_defaults[i],
            step=5,
            key=f"mix_{i}",
            help=f"Percentage share of {at} assets (0–100). "
                 f"Controls estate composition — ICS-heavy vs cloud-heavy etc.")
    # Show total
    _mix_total = sum(mix_raw.values())
    if abs(_mix_total - 100) > 5:
        st.caption(f"⚠️ Mix total: {_mix_total}% (ideally 100%)")
    else:
        st.caption(f"✅ Mix total: {_mix_total}%")
    ...
    _actors_default = st.session_state.get("_actors_preset",
                       ["APT Group", "Cybercriminal", "Insider Threat"])
    selected_actors = st.multiselect("Active actors:",
        list(THREAT_ACTORS.keys()),
        default=_actors_default,
    ...
    n_scenarios = st.number_input(
        "Scenarios to generate", min_value=200, max_value=10000,
        value=int(st.session_state.get("_n_scenarios_preset", 1000)), step=100,
        help="Number of synthetic attack scenarios to generate. More = better model but slower training.")
    ...
    mc_n_sims     = st.slider("Monte-Carlo simulations", 10, 500, int(st.session_state.get("_mc_n_sims_preset", 100)), 10,
    ...
    bench_max = st.number_input("Max N for benchmark", value=int(st.session_state.get("_bench_max_preset", 500)), step=50,
    ...
    bench_pts = st.slider("N points", 4, 12, int(st.session_state.get("_bench_pts_preset", 5)),
...
with tab_step6:
    ...
    with lab_tabs[0]:
        ...
        with msc_a:
            n_seeds = st.slider("Number of seeds", 2, 10, 2, key="ms_n_seeds_v1",
        ...
        with msc_b:
            _est_sec = len(bench_sizes) * n_seeds * 12  # ~12s per config
            _est_min = _est_sec / 60
            st.caption(f"Total runs: {len(bench_sizes) * n_seeds} · "
                       f"Est. time: ~{_est_min:.0f} min")
        # Fast mode / Full mode toggle
        _fcol1, _fcol2 = st.columns([2, 1])
        with _fcol2:
            if st.button("⚡ Fast Mode", key="fast_mode_btn",
                         help="Sets Seeds=2, Max N=300, Points=4 — runs in ~90 seconds. Sufficient to prove O(N^1.4)."):
                st.session_state["ms_n_seeds_v1"] = 2
                st.session_state["_bench_max_preset"] = 300
                st.session_state["_bench_pts_preset"] = 4
                st.rerun()
        ...
        if st.button("▶ Run multi-seed benchmark", key="run_multi_seed_v1", type="primary"):
            ...
            total_configs = len(bench_sizes) * len(seeds_to_run)

            # ── Live progress bar ─────────────────────────────────────────────
            _prog_bar  = st.progress(0, text="Starting benchmark…")
            _prog_status = st.empty()
            _prog_table  = st.empty()
            _done = 0
            _all_records = []
            _t_start = time.time()
            ...
            for _seed in seeds_to_run:
                for _n in bench_sizes:
                    ...
                    _pct = _done / total_configs
                    _eta = (_elapsed / _done * (total_configs - _done)) if _done > 0 else 0
                    _prog_bar.progress(
                        _pct,
                        text=f"Running N={_n}, Seed={_seed} "
                             f"({_done}/{total_configs}) — "
                             f"elapsed {_elapsed:.0f}s, "
                             f"ETA ~{_eta:.0f}s")
                    _prog_status.markdown(
                        f"⚡ **N = {_n}** &nbsp;|&nbsp; "
                        f"Seed {_seed} &nbsp;|&nbsp; "
                        f"Run {_done+1} of {total_configs}")
                    try:
                        _df_one = run_scalability_benchmark(
                            [_n], json.dumps(asset_mix),
                            tuple(selected_actors), tuple(selected_vecs),
                            int(_seed), rf_params, gb_params,
                        )
                        _df_one["seed"] = int(_seed)
                        _all_records.append(_df_one)
                        # Show live results table
                        _live_rows = pd.concat(_all_records, ignore_index=True)
                        _prog_table.dataframe(
                            _live_rows[["N","seed","gen_time","scen_time",
                                        "feat_time","ml_time","total_time"]].round(3),
                            use_container_width=True, hide_index=True)
                    except Exception as _exc:
                        st.warning(f"Seed {_seed}, N={_n} failed: {_exc}")
                    _done += 1

            _prog_bar.progress(1.0, text=f"✅ Done! {total_configs} configs in {time.time()-_t_start:.0f}s")
            _prog_status.empty()

            if _all_records:
                long_df = pd.concat(_all_records, ignore_index=True)
                ...
                st.success(f"✅ Benchmark complete — {len(long_df)} runs across "
                           f"{long_df['N'].nunique()} N values × "
                           f"{long_df['seed'].nunique()} seeds")
            else:
                st.error("All benchmark runs failed.")
    ...
    with lab_tabs[8]:
        ...
        if not fits_details or "FULL PIPELINE" not in fits_details:
            ...
            if not isinstance(fits_details, dict): fits_details = {}
            tot_fit = fits_details.get("FULL PIPELINE",
                      fits_details.get("full_pipeline",
                      next(iter(fits_details.values()), None) if fits_details else None))
            if tot_fit is None: tot_fit = {}
    ...
    with lab_tabs[9]:
        ...
        fits_details = st.session_state.get("bench_complexity_fits")
        if not isinstance(fits_details, dict):
            fits_details = {}
        tot = fits_details.get("FULL PIPELINE", fits_details.get("full_pipeline",
              next(iter(fits_details.values()), {}) if fits_details else {}))
        ...
        if st.button("🚀 Build full bundle (one click)", key="build_live_bundle_btn",
            ...
            try:
                with st.spinner("Fetching KEV + EPSS + NVD and building the bundle…"):
                    ...
                    # Never overwrite a full simulated env with the live bundle env.
                    # The live bundle overlays CVE data onto the existing asset portfolio.
                    # The simulated env (Step 2) is preserved in session_state["env"].
                    if st.session_state.get("env") is None:
...
CVSS: https://www.first.org/cvss/""", language="text")
    ...
    if isinstance(kaggle_df, pd.DataFrame) and not kaggle_df.empty:
        ...
        if st.button("🔗 Merge Kaggle CVEs into active vulnerability bundle",
            ...
            _bundle_assets = bundle.get("assets")
            if isinstance(_bundle_assets, pd.DataFrame) and not _bundle_assets.empty:
                base_assets = _bundle_assets
            else:
                base_assets = (st.session_state.get("env") or {}).get("assets", pd.DataFrame())
    ...
    if bundle:
        ...
        if st.button("✅ Use uploaded assets as active environment", key="apply_real_env"):
            ...
            if env_real:
                # Only overwrite env if no simulated env exists
                if st.session_state.get("env") is None:
...
_DPI_RENDER  = 170   # Render DPI: 170×(300/170)=2379px > 2244px ESWA minimum ✅
...
_DPI             = 170    # default render DPI

_FW_SINGLE  = 3.54  # inches — single column (90mm Elsevier spec)
_FW_RENDER  = 7.87  # inches — render width (wider to compensate bbox_tight clipping)
_FW_DOUBLE  = 7.48  # inches — double column (190mm Elsevier spec, for display only)
_FW_WIDE    = 7.87  # inches — full width (= _FW_RENDER for ZIP figures)
...
def _savefig(fig, dpi=None):
    """
    Render figure at _DPI_RENDER (170 DPI) for memory safety on Streamlit Cloud.
...
def _rt(df, title=""):
    ...
    try:
        ...
        fw = min(_FW_RENDER, max(_FW_RENDER, 1.1 * nc))
...
def _rs(x, y, title, xl, yl):
    ...
    try:
        plt, cm = _mpl()
        fig, ax = plt.subplots(figsize=(_FW_RENDER, 5))
...
def _rb(labels, values, title, xl, horizontal=True):
    ...
    try:
        ...
        if horizontal:
            ...
            fw = max(_FW_RENDER, 0.7*len(labels))
...
def _rh(vals, vline, title, xl):
    ...
    try:
        plt, cm = _mpl()
        fig, ax = plt.subplots(figsize=(_FW_RENDER, 3.8))
...
def _rl(x, y_dict, title, xl, yl, log_axes=False):
    ...
    try:
        plt, cm = _mpl()
        styles = ["-","--","-.",":","--","-"]
        fig, ax = plt.subplots(figsize=(_FW_RENDER, 4))
...
def _compare_bars(run_labels, metric_dict, title, ylabel):
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(max(_FW_RENDER, 1.1*n_mod), 5))
...
with tab_claude:
    st.subheader("✨ Claude AI — Test, Save & Compare")
...
1. **Set inputs in sidebar** (assets, seed, asset mix, threat actors, scenarios)
2. **Run pipeline** → Steps 2 → 3 → 4 → 5 → 5b → 6
3. **Save Run** below → repeat with different inputs → **Generate Analysis**

**Recommended test sequence:**
| Run | Assets | Seed | Asset Mix | Actors | Scenarios | Purpose |
|---|---|---|---|---|---|---|
| 1 | 300 | 42 | Mixed (default) | APT + Insider | 1,000 | Primary baseline |
| 2 | 300 | 123 | Mixed (default) | APT + Insider | 1,000 | Seed stability |
| 3 | 300 | 42 | ICS 40%, DB 25%, Net 20% | APT + Nation State | 1,000 | Critical infra |
| 4 | 300 | 42 | Mixed (default) | APT only | 1,000 | High-threat actor |
| 5 | 300 | 42 | Mixed (default) | APT + Insider | 200 | Data scarce |
| 6 | 1,000 | 42 | Mixed (default) | APT + Insider | 3,000 | Enterprise scale |
        """)
    ...
    sc1.metric("Step 2 Env",   "✅" if isinstance(ss.get("env"), dict) else "⚠️ Not run")
    sc2.metric("Step 3 Scen",  "✅" if isinstance(ss.get("scenarios"), pd.DataFrame) else "⚠️ Not run")
    sc3.metric("Step 4 Feat",  "✅" if isinstance(ss.get("features"),  pd.DataFrame) else "⚠️ Not run")
    sc4.metric("Step 5 ML",    "✅" if isinstance(ss.get("ml_results"), dict) and ss.get("ml_results") else "⚠️ Not run")
    sc5.metric("Step 5b Alert","✅" if isinstance(ss.get("clf_results"), dict) and ss.get("clf_results") else "⚠️ Not run")
    sc6.metric("Step 6 Scale", "✅" if (isinstance(ss.get("bench_multi_seed_agg"), pd.DataFrame)
                                        or isinstance(ss.get("bench_complexity_fits"), dict)) else "⚠️ Optional")

    # ── CURRENT INPUT SUMMARY ─────────────────────────────────────────────────
    st.markdown("#### ⚙️ Current Sidebar Inputs")
    env_now = ss.get("env") if isinstance(ss.get("env"), dict) else {}
    sc_now  = ss.get("scenarios")
    ml_now  = ss.get("ml_results") or {}

    # Read actual live sidebar values
    _n_assets_now  = env_now.get("n_assets", n_assets)
    _seed_now      = env_now.get("seed", rng_seed) if "seed" in env_now else rng_seed
    _n_scen_now    = _safe_len(sc_now)
    _actors_now    = env_now.get("actor_types", selected_actors) if "actor_types" in env_now else selected_actors
    _mix_now       = env_now.get("asset_mix", asset_mix) if "asset_mix" in env_now else asset_mix
    _best_r2_now   = "—"
    _best_model_now = "—"
    if ml_now:
        try:
            _ml_only = {n: r for n,r in ml_now.items() if isinstance(r,dict) and r.get("model_kind")=="ml"}
            if _ml_only:
                _best_model_now = max(_ml_only, key=lambda n: float(_ml_only[n].get("r2",-1)))
                _best_r2_now    = f"{_ml_only[_best_model_now].get('r2','?'):.4f}"
        except Exception:
            pass

    i1,i2,i3,i4,i5,i6 = st.columns(6)
    i1.metric("Assets",    str(_n_assets_now))
    i2.metric("Seed",      str(_seed_now))
    i3.metric("Scenarios", str(_n_scen_now) if _n_scen_now > 0 else "—")
    i4.metric("Best R²",   _best_r2_now)
    i5.metric("Best Model",_best_model_now[:12] if _best_model_now != "—" else "—")
    i6.metric("Actors",    str(len(_actors_now)) + " active" if _actors_now else "—")

    # Show asset mix
    if _mix_now:
        st.caption("**Asset mix:** " + " | ".join(f"{k}: {v}%" for k,v in _mix_now.items()))

    st.divider()

    # ── SAVE CURRENT RUN ──────────────────────────────────────────────────────
    st.markdown("#### 💾 Step 1 — Save Current Run")
    st.caption("Give this run a meaningful label that describes what you changed. "
               "e.g. 'Run1_Baseline', 'Run2_ICSHeavy', 'Run3_APTOnly'")

    # Auto-generate label from actual current inputs
    _mix_str = "Mixed"
    if _mix_now:
        top2 = sorted(_mix_now.items(), key=lambda x: x[1], reverse=True)[:2]
        _mix_str = "+".join(k.split()[0] for k,v in top2)
    _actor_str = "+".join(a.split()[0] for a in (_actors_now or ["APT"])[:2])
    _auto_label = f"{_n_assets_now}N_{_mix_str}_{_actor_str}_S{_seed_now}_Sc{_n_scen_now}"

    c_lbl, c_btn = st.columns([3, 1])
    with c_lbl:
        run_label = st.text_input("Run label", value=_auto_label,
                                  placeholder="e.g. 'Run1_300N_Baseline_Seed42'",
                                  key="run_label_input")
    with c_btn:
        st.markdown("<br>", unsafe_allow_html=True)
        save_btn = st.button("💾 Save This Run", type="primary",
                             use_container_width=True, key="save_run_btn")

    if save_btn:
        if not run_label.strip():
            st.error("Please enter a label.")
        else:
            # Capture ALL sidebar inputs
            current_cfg = {
                "Assets (N)":       _n_assets_now,
                "Random Seed":      _seed_now,
                "Scenarios":        _n_scen_now,
                "Test Split %":     int(test_size * 100),
                "CV Folds":         cv_folds,
                "MC Simulations":   mc_n_sims,
                "Threat Actors":    ", ".join(_actors_now) if _actors_now else "APT",
                "Asset Mix":        " | ".join(f"{k}:{v}%" for k,v in (_mix_now or {}).items()),
                "Attack Vectors":   ", ".join(selected_vecs[:3]) + ("…" if len(selected_vecs)>3 else ""),
                "Live Data":        "Yes" if isinstance(ss.get("real_data_bundle"), dict) else "No",
                "Kaggle CVE":       "Yes" if isinstance(ss.get("kaggle_clf_results"), dict) else "No",
            }
            ...
            if snp["n_figures"] > 0:
                st.success(
                    ...
                    f"{snp['n_figures']} figures captured | "
                    f"{len(set(s for s,_,_ in snp['figures']))} sections | "
                    f"Best R²: {snp['metrics'].get('best_r2','—')} | "
                    f"Best Model: {snp['metrics'].get('best_model','—')}")
            else:
                st.error(f"⚠️ 0 figures captured — run pipeline steps first.")
                for err in snp.get("errors", [])[:5]:
                    st.code(err)

    st.divider()

    # ── SAVED RUNS COMPARISON TABLE ───────────────────────────────────────────
    st.markdown("#### 📊 Step 2 — Saved Runs & Comparison")

    runs = ss.get("saved_runs", [])

    if not runs:
        st.info("No runs saved yet. Run the pipeline → Save This Run → repeat with different inputs.")
    else:
        # Build comparison table from all saved runs
        comp_rows = []
        for r in runs:
            cfg = r.get("config", {})
            met = r.get("metrics", {})
            row = {
                ...
                "Assets":       cfg.get("Assets (N)", "—"),
                "Seed":         cfg.get("Random Seed", "—"),
                "Scenarios":    cfg.get("Scenarios", "—"),
                "Actors":       cfg.get("Threat Actors", "—")[:25],
                "Asset Mix":    cfg.get("Asset Mix", "—")[:30],
                "Live Data":    cfg.get("Live Data", "—"),
                "Best R²":      f"{met.get('best_r2', '—'):.4f}" if isinstance(met.get('best_r2'), float) else "—",
                "Best Model":   met.get("best_model", "—"),
                "Best MAE":     f"{met.get('best_mae', '—'):.4f}" if isinstance(met.get('best_mae'), float) else "—",
                "Best F1":      f"{met.get('best_clf_f1', '—'):.4f}" if isinstance(met.get('best_clf_f1'), float) else "—",
                "Best AUC":     f"{met.get('best_clf_auc', '—'):.4f}" if isinstance(met.get('best_clf_auc'), float) else "—",
                "Risk Mean":    f"{met.get('risk_mean', '—'):.3f}" if isinstance(met.get('risk_mean'), float) else "—",
                "Risk Std":     f"{met.get('risk_std', '—'):.3f}" if isinstance(met.get('risk_std'), float) else "—",
                "SHAP Top":     met.get("shap_top_feature", "—"),
                "KEV Slope":    f"{met.get('slope_scenario_gen', '—'):.3f}" if isinstance(met.get('slope_scenario_gen'), float) else "—",
        ...
        if len(runs) > 1:
            ...
            with qc1:
                # R² comparison
                labels_q = [r["label"][:20] for r in runs]
                r2_vals  = [r["metrics"].get("best_r2", 0) or 0 for r in runs]
                fig_q1 = go.Figure(go.Bar(
                    x=labels_q, y=r2_vals,
                    marker_color=["#1a73e8" if i==0 else "#34A853" if i==1
                                  else "#EA4335" if i==2 else "#FBBC04"
                                  for i in range(len(runs))],
                    text=[f"{v:.4f}" for v in r2_vals], textposition="auto"))
                fig_q1.update_layout(title="Best R² per Run", height=300,
            ...
            with qc2:
                # F1 comparison
                f1_vals = [r["metrics"].get("best_clf_f1", 0) or 0 for r in runs]
                fig_q2 = go.Figure(go.Bar(
                    x=labels_q, y=f1_vals,
                    marker_color=["#9C27B0" if i%2==0 else "#00BCD4"
                                  for i in range(len(runs))],
                    text=[f"{v:.4f}" for v in f1_vals], textposition="auto"))
                fig_q2.update_layout(title="Best F1 (Alerting) per Run", height=300,
                    margin=dict(l=0,r=0,t=40,b=80),
                    xaxis_tickangle=-30, yaxis_range=[0, 1])
                st.plotly_chart(fig_q2, use_container_width=True)

            with qc3:
                # Mean Risk Score comparison
                risk_vals = [r["metrics"].get("risk_mean", 0) or 0 for r in runs]
                risk_stds = [r["metrics"].get("risk_std", 0) or 0 for r in runs]
                fig_q3 = go.Figure(go.Bar(
                    x=labels_q, y=risk_vals,
                    error_y=dict(type="data", array=risk_stds, visible=True),
                    marker_color=["#FF5722" if i%2==0 else "#FF9800"
                                  for i in range(len(runs))],
                    text=[f"{v:.3f}" for v in risk_vals], textposition="auto"))
                fig_q3.update_layout(title="Mean Risk Score ± Std", height=300,
                    margin=dict(l=0,r=0,t=40,b=80), xaxis_tickangle=-30)
                st.plotly_chart(fig_q3, use_container_width=True)

        # Delete controls
        st.markdown("**Manage runs:**")
        del_cols = st.columns(min(len(runs), 4))
        for i, r in enumerate(runs):
            with del_cols[i % 4]:
                if st.button(f"🗑️ Delete '{r['label'][:15]}'",
                             key=f"del_{i}", use_container_width=True):
                    ss["saved_runs"].pop(i)
                    st.rerun()
        if st.button("🗑️ Clear All Runs", key="clear_all"):
            ss["saved_runs"] = []
            st.rerun()
    ...
    st.markdown("#### 🧠 Step 3 — Generate Full Paper Analysis")

    n_runs = len(runs)
    if n_runs == 0:
        st.info("Save at least one run to enable analysis.")
    elif n_runs == 1:
        st.info("💡 Save more runs with different inputs to get cross-run comparison charts.")
    else:
        st.success(f"🎯 **{n_runs} runs** ready — Claude will compare all runs and "
                   f"generate cross-run comparison charts showing how inputs change outputs.")

    total_figs = sum(r.get("n_figures", 0) for r in runs)

    if n_runs > 0:
        st.markdown(f"**Total:** {n_runs} run(s) · {total_figs} figures")

        gen_btn = st.button(
            f"🚀 Generate Complete Paper Analysis ({n_runs} run(s), {total_figs} figures)",
            type="primary", use_container_width=True, key="gen_analysis_btn",
            disabled=(n_runs == 0))

        if gen_btn:
            with st.spinner("Building comparison charts and sending to Claude… (~60 seconds)"):
                try:
                    import base64 as _b64_api

                    # Build comparison figures
                    comp_figs = _build_comparison_figures(runs)

                    # Select 20 most informative images for API
                    PRIORITY = ["Model Comparison","R² Bar","Predicted vs Actual",
                                "SHAP","Permutation","ROC Curves","Classifier Metrics",
                                "Risk Score Dist","Risk by Asset","Scalability",
                                "Complexity","Live Bundle","Kaggle","Feature Corr",
                                "Threat Likelihood","CVSS Dist","Attack Vector",
                                "MITRE Tactic","Severity","Exposure"]

                    def _is_pri(label):
                        return any(p.lower() in label.lower() for p in PRIORITY)

                    api_imgs = []
                    # Comparison charts first (max 6)
                    for label, b64 in comp_figs[:6]:
                        api_imgs.append((f"[COMPARISON] {label}", b64))

                    # Key figures from each run
                    max_per = max(1, (20 - len(api_imgs)) // max(n_runs, 1))
                    for r in runs:
                        pri  = [(s,l,b) for s,l,b in r["figures"] if _is_pri(l)]
                        rest = [(s,l,b) for s,l,b in r["figures"] if not _is_pri(l)]
                        sel  = (pri + rest)[:max_per]
                        for sec, label, b64 in sel:
                            api_imgs.append((f"[{r['label']}] {label}", b64))

                    api_imgs = api_imgs[:20]

                    # Downscale for API (max 1800px)
                    content = []
                    for label, b64 in api_imgs:
                        content.append({
                            "type": "image",
                            "source": {"type":"base64","media_type":"image/png",
                                       "data": _downscale_for_api(b64)}
                        })

                    prompt = _master_prompt(runs, comp_figs)
    ...
    if ss.get("_paper_analysis"):
        ...
        st.success(f"✅ Analysis complete — {nr} run(s), {nf} figures · {ts}")
        st.divider()
        ...
        with d1:
            st.download_button("📄 Paper Draft (.md)",
                data=(f"# PASTA-ML Paper Analysis\n**Runs:** {nr} | "
                      f"**Generated:** {ts}\n\n---\n\n{analysis}").encode(),
                file_name=f"pasta_ml_paper_{stamp}.md", mime="text/markdown",
                use_container_width=True)
        with d2:
            st.download_button("📥 Paper Draft (.txt)",
                data=analysis.encode(),
                file_name=f"pasta_ml_paper_{stamp}.txt", mime="text/plain",
                use_container_width=True)
        with d3:
            export = {"generated":ts,"n_runs":nr,"n_figures":nf,
                      "run_labels":[r["label"] for r in runs],"analysis":analysis}
            st.download_button("🗂️ With Metadata (.json)",
                data=__import__("json").dumps(export, indent=2).encode(),
                file_name=f"pasta_ml_paper_{stamp}.json", mime="application/json",
                use_container_width=True)

    st.divider()

    # ── FIGURE ZIP ────────────────────────────────────────────────────────────
    st.markdown("#### 📦 Download All Figures (ESWA-Compliant)")
    st.caption("All figures upsampled to 300–500 DPI (Elsevier ESWA minimum). "
               "Organised by run and section. Insert directly into your submission.")

    if st.button("📥 Build Figure ZIP", key="build_zip_btn", use_container_width=True):
        if not runs:
            st.error("No runs saved yet.")
        else:
            with st.spinner("Packaging all figures…"):
                try:
                    import base64 as _b64_zip, io as _io_zip
                    import zipfile as _zf_zip, gc as _gc_zip

                    def _eswa_dpi(label):
                        ll = label.lower()
                        if any(x in ll for x in ["table","stats","summary","metrics table",
                                                  "comparison table","benchmark table",
                                                  "complexity table","dataset","bundle"]):
                            return _DPI_COMBINATION
                        if any(x in ll for x in ["scatter","actual","distribution","histogram",
                                                  "dist","perm","epss cvss"]):
                            return _DPI_HALFTONE
                        return _DPI_LINE
                    ...
                    with _zf_zip.ZipFile(zip_buf, "w", _zf_zip.ZIP_DEFLATED) as zf:
                        ...
                        for i, (label, b64) in enumerate(comp_figs_now):
                            fname = (f"00_Comparison/"
                                     f"Fig_{i+1:02d}_{label.replace(' ','_')[:50]}.png")
                            b64p = _upsample_to_eswa(b64, _DPI_LINE)
                            zf.writestr(fname, _b64_zip.b64decode(b64p))
                            del b64p
                            idx_rows.append({"Run":"ALL","Section":"Comparison",
                                             "Label":label,"DPI":_DPI_LINE,"File":fname})
                            fig_count += 1

                        # Per-run figures
                        for r in runs:
                            run_name  = r["label"].replace(" ","_")[:35]
                            prev_sec  = ""; sec_idx = 0; fig_idx = 0
                            for sec, label, b64 in r["figures"]:
                                if sec != prev_sec:
                                    sec_idx += 1; fig_idx = 0; prev_sec = sec
                                fig_idx += 1
                                safe_sec   = sec.replace(":","").replace(" ","_")[:25]
                                safe_label = label.replace(" ","_").replace("/","_")[:40]
                                fname = (f"{run_name}/"
                                         f"{sec_idx:02d}_{safe_sec}/"
                                         f"Fig_{fig_idx:02d}_{safe_label}.png")
                                tdpi = _eswa_dpi(label)
                                b64p = _upsample_to_eswa(b64, tdpi)
                                zf.writestr(fname, _b64_zip.b64decode(b64p))
                                del b64p
                                idx_rows.append({"Run":r["label"],"Section":sec,
                                                 "Label":label,"DPI":tdpi,"File":fname})
                                fig_count += 1
                                if fig_count % 5 == 0:
                                    _gc_zip.collect()

                        # Figure index
                        zf.writestr("FIGURE_INDEX.csv",
                                    pd.DataFrame(idx_rows).to_csv(index=False))

                        # Run comparison table
                        if comp_rows:
                            zf.writestr("RUN_COMPARISON.csv",
                                        pd.DataFrame(comp_rows).to_csv(index=False))

                        # Paper analysis if available
                        if ss.get("_paper_analysis"):
                            zf.writestr("PAPER_ANALYSIS.md",
                                        f"# PASTA-ML Paper Analysis\n"
                                        f"Generated: {ss.get('_paper_analysis_ts','')}\n\n"
                                        f"---\n\n{ss['_paper_analysis']}")

                    zip_buf.seek(0)
                    ss["_zip_bytes"] = zip_buf.read()
                    st.success(f"✅ ZIP ready — {fig_count} figures, "
                               f"{len(comp_figs_now)} comparison charts")

                except Exception as _ze:
                    st.error(f"ZIP build failed: {_ze}")

    if ss.get("_zip_bytes"):
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        st.download_button(
            f"⬇️ Download pasta_ml_figures_{stamp}.zip",
            data=ss["_zip_bytes"],
            file_name=f"pasta_ml_figures_{stamp}.zip",
            mime="application/zip",
            type="primary", use_container_width=True,
            help=("Contains: 00_Comparison/ (cross-run charts) | "
                  "RunName/Section/ (per-run figures) | "
                  "FIGURE_INDEX.csv | RUN_COMPARISON.csv | PAPER_ANALYSIS.md"))
