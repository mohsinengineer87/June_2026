...
_DPI_HALFTONE    = 300    # target DPI for ZIP export (halftone)
_DPI_COMBINATION = 500    # target DPI for ZIP export (tables)
_DPI_LINE        = 1000   # target DPI for ZIP export (line art)
...
def _upsample_to_eswa(b64_png, target_dpi, fig_type="line_art"):
    ...
    try:
        ...
        new_w   = int(w * scale)
        new_h   = int(h * scale)
        # Use LANCZOS for crisp lines, especially at 1000 DPI
...
with tab_claude:
    ...
    if ss.get("_paper_analysis"):
        ...
        st.caption(
            "Every chart from every tab — upsampled to ESWA spec (300/500/1000 DPI by type). "
            "Download the ZIP and insert figures directly into your ESWA submission.")
