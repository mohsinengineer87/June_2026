...
def _fig_to_b64(fig):
    """Render a Plotly figure to base64 PNG for Claude vision API."""
    try:
        buf = _io.BytesIO()
...
def _build_auto_images():
    ...
    if isinstance(ml, dict) and ml:
        ...
        b64 = _fig_to_b64(_df_to_fig(df_ml, "Step 5 — ML Regression Model Comparison"))
        if b64:
    ...
    if isinstance(clf, dict) and clf and "error" not in clf:
        ...
        b64 = _fig_to_b64(_df_to_fig(df_clf, "Step 5b — Alerting Classifier Comparison"))
        if b64:
    ...
    bench = ss.get("bench_results")
    if bench is not None and isinstance(bench, pd.DataFrame) and len(bench) > 0:
        # Table
        b64 = _fig_to_b64(_df_to_fig(bench.round(3), "Step 6 — Scalability Benchmark"))
        if b64:
            images.append(("Scalability Benchmark Table", b64))

        # Log-log chart
        time_cols = [c for c in bench.columns
                     if any(x in c.lower() for x in ["time", "scenario", "feature", "train"])]
        if "n_assets" in bench.columns and time_cols:
            fig_ll = go.Figure()
            clrs   = ["#1a73e8","#34A853","#EA4335","#FBBC04"]
            for col, clr in zip(time_cols, clrs):
                try:
                    x = bench["n_assets"].values.astype(float)
                    y = bench[col].values.astype(float)
                    mask = (x > 0) & (y > 0)
            ...
            fig_ll.update_layout(
                title="Scalability — Runtime by Phase (log-log)",
                xaxis_title="N (Assets)", yaxis_title="Time (s)",
                xaxis_type="log", yaxis_type="log",
                height=460, margin=dict(l=0, r=0, t=50, b=0))
            b64 = _fig_to_b64(fig_ll)
            if b64:
...
with tab_claude:
    ...
    has_bench = isinstance(ss.get("bench_results"), pd.DataFrame)
