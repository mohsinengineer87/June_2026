...
# ─────────────────────────────────────────────────────────────────────────────
# RENDERING HELPERS  (pure matplotlib — no kaleido, works on Streamlit Cloud)
# ─────────────────────────────────────────────────────────────────────────────

# ── ESWA publication settings ────────────────────────────────────────────────
# ESWA requires 300 DPI minimum, Times New Roman or similar serif font,
# minimum 8pt font size, single-column ~3.5 inch / double-column ~7 inch wide.
# All figures rendered at 300 DPI, 7-inch double-column width.

_DPI        = 300          # ESWA minimum
_FW_SINGLE  = 3.5          # inches — single column
_FW_DOUBLE  = 7.0          # inches — double column (default)
_FW_WIDE    = 9.0          # inches — full-width tables
_FONT_BASE  = 9            # pt — axis labels (≥8pt required)
_FONT_TITLE = 10           # pt — figure title
_FONT_TICK  = 8            # pt — tick labels
_FONT_LEG   = 8            # pt — legend

# ESWA-compliant colour palette (colourblind-safe)
_C = ["#1a73e8","#34A853","#EA4335","#FBBC04","#9C27B0","#00BCD4","#FF9800"]

def _savefig(fig, dpi=_DPI):
    """Save figure to base64 PNG at publication DPI."""
    buf = io.BytesIO()
...
def _apply_eswa_style(ax, title, xl="", yl=""):
    """Apply consistent ESWA-compliant styling to an axes."""
...
def _rt(df, title=""):
    """DataFrame → 300 DPI publication-quality table PNG."""
    try:
        df = df.copy()
...
def _rs(x, y, title, xl, yl):
    """Scatter plot → 300 DPI PNG (double-column width)."""
    try:
...
def _rb(labels, values, title, xl, horizontal=True):
    ...
    try:
        ...
        if horizontal:
            ...
            colors = [plt.cm.RdYlGn_r(n) for n in norm]
...
def _rh(vals, vline, title, xl):
    """Histogram → 300 DPI PNG."""
    try:
        fig, ax = plt.subplots(figsize=(_FW_DOUBLE, 3.8))
        n, bins, patches = ax.hist(vals, bins=35, color=_C[0],
...
def _roc(clf_dict):
    """ROC curves → 300 DPI PNG (all classifiers on one plot)."""
    try:
        ...
        for (nm, r), col, sty in zip(clf_dict.items(), _C, styles):
            if isinstance(r, dict) and "y_test" in r and "y_proba" in r:
                try:
                    fpr, tpr, _ = _rc(np.array(r["y_test"]),
                                      np.array(r["y_proba"]))
                    auc = r.get("roc_auc", 0)
...
def _compare_bars(run_labels, metric_dict, title, ylabel):
    """Grouped bar — compare metric across runs → 300 DPI PNG."""
    try:
...
def _compare_lines(run_labels, x_vals_list, y_vals_list, title, xl, yl,
                   log_axes=False):
    """Overlay lines from multiple runs → 300 DPI PNG."""
    try:
        styles = ["-","--","-.",":","--"]
        fig, ax = plt.subplots(figsize=(_FW_DOUBLE, 4))
...
with tab_claude:
    ...
    col1.metric("Step 2", "✅" if ss.get("env")          else "⚠️")
    col2.metric("Step 3", "✅" if ss.get("scenarios") is not None else "⚠️")
    col3.metric("Step 4", "✅" if ss.get("features")  is not None else "⚠️")
    col4.metric("Step 5", "✅" if ss.get("ml_results")    else "⚠️")
    col5.metric("Step 5b","✅" if ss.get("clf_results")   else "⚠️")
    col6.metric("Step 6", "✅" if (ss.get("bench_multi_seed_agg") is not None
                                   or ss.get("bench_complexity_fits")) else "⚠️")
    ...
    with c_label:
        ...
        auto_label = f"{env_n} Assets, Seed 42, {len(ss.get('scenarios', []) or [])} Scenarios"
    ...
    if save_btn:
        if not run_label.strip():
            ...
            current_cfg = {
                ...
                "Scenarios":   len(ss["scenarios"]) if ss.get("scenarios") is not None else "?",
                "Seed":        42,
                "Test Split":  "20%",
                "CV Folds":    5,
        ...
        if st.button(
            ...
            with st.spinner("Building comparison charts and sending to Claude…"):
                try:
                    ...
                    # Build API content: comparison figs first, then per-run
                    content = []
                    for label, b64 in comp_figs:
                        content.append({
                            "type": "image",
                            "source": {"type":"base64",
    ...
    if ss.get("_paper_analysis"):
        ...
        st.caption(
            "Every chart from every tab, rendered at 300 DPI (ESWA minimum). "
            "Download as a ZIP file and insert directly into your paper PDF.")
