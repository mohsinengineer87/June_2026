...
with st.sidebar:
    ...
    # ── ONE-CLICK RUN PRESETS ─────────────────────────────────────────────────
    st.markdown("### 🎯 Quick Run Presets")
    st.caption("Click a preset to auto-fill all sidebar inputs for that experiment.")
    ...
    _PRESETS = {
        "Financial Services (Bank/FinTech)": {
            "emoji": "🏦",
            "label": "🏦 Financial\nServices",
            "desc": "Bank / FinTech: DB & Cloud heavy, Cybercriminal + Insider threats",
            "detail": "High-value transaction & data systems. PCI-DSS regulated. Primary threats: financial fraud, data exfiltration, ransomware. No ICS/OT exposure.",
            "n_assets": 300, "rng_seed": 42,
            "mix":    [20,  5, 35, 10, 20,  0, 10],
            "actors": ["Cybercriminal", "Insider Threat", "APT Group"],
            "n_scenarios": 1000, "mc_n_sims": 200, "bench_max": 5000,
            "note": "",
        },
        "Critical Infrastructure (Energy/ICS)": {
            "emoji": "🏭",
            "label": "🏭 Critical\nInfrastructure",
            "desc": "Energy / Utilities: ICS & SCADA heavy, Nation-State + APT threats",
            "detail": "Operational technology (OT/ICS/SCADA), power grids, water treatment. Primary threats: nation-state sabotage, ransomware disruption, firmware implants.",
            "n_assets": 300, "rng_seed": 42,
        ...
        "Healthcare + Live Data (Hospital/Pharma)": {
            "emoji": "🏥",
            "label": "🏥 Healthcare\n+ Live Data",
            "desc": "Hospital / Pharma: IoT medical devices, HIPAA, Live CISA KEV + Kaggle CVEs",
            "detail": "Medical IoT (infusion pumps, imaging), patient records, life-critical endpoints. Validates with real CVE data from CISA KEV, FIRST EPSS, NVD and 330k Kaggle CVEs.",
            "n_assets": 300, "rng_seed": 42,
            "mix":    [15, 30, 20, 10, 15,  5,  5],
            "actors": ["Cybercriminal", "APT Group", "Insider Threat"],
            "n_scenarios": 1000, "mc_n_sims": 200, "bench_max": 5000,
            "note": "\u26a0\ufe0f For full RQ3 validation: Real Data + CTI tab \u2192 Build live bundle (CISA KEV + FIRST EPSS + NVD) \u2192 Upload Kaggle CSV \u2192 Train CVE classifier \u2014 BEFORE running pipeline.",
        },
    }

    def _apply_preset(p, name=""):
        st.session_state["_n_assets_preset"]    = p["n_assets"]
        st.session_state["_rng_seed_preset"]    = p["rng_seed"]
        st.session_state["_n_scenarios_preset"] = p["n_scenarios"]
        st.session_state["_mc_n_sims_preset"]   = p["mc_n_sims"]
        st.session_state["_bench_max_preset"]   = min(p["bench_max"], 10000)
        st.session_state["_actors_preset"]      = p["actors"]
        for i, v in enumerate(p["mix"]):
            st.session_state[f"mix_{i}"] = v
        st.session_state["_preset_active"] = True
        st.session_state["_preset_note"]   = p.get("note", "")
        st.session_state["_preset_name"]   = p.get("label", name).replace("\n", " ")

    _pc = st.columns(3)
    for _col, (_name, _preset) in zip(_pc, _PRESETS.items()):
        with _col:
            _lbl = _preset.get("label", _name).replace("\n", "\n")
            if st.button(_lbl,
                         key=f"preset_btn_{_name[:20]}",
                         use_container_width=True,
                         help=f"{_preset['desc']}\n\n{_preset['detail']}"):
                _apply_preset(_preset, name=_name)
                st.rerun()

    if st.session_state.get("_preset_active"):
        _pname = st.session_state.get("_preset_name", "")
        _pnote = st.session_state.get("_preset_note", "")
        if _pnote:
            st.warning(f"\u2705 **{_pname}** loaded.\n\n{_pnote}")
        else:
            st.success(
                f"\u2705 **{_pname}** loaded \u2014 "
                f"inputs set. Run Steps 2 \u2192 3 \u2192 4 \u2192 5 \u2192 5b \u2192 Save Run.")
        if st.button("\u2716 Clear", key="clear_preset_btn"):
